...
def gen_data(source, s_pad, Y, y_pad, max_length, vocsize, nclasses, n_batch=5):
    ...
    for i in range(len(source)):
        ...
        yield (seq_lengths, np.array(X), np.array(y))
...
input_x = tf.placeholder(tf.int32, shape=[batch_size, max_sentence_length])
input_y = tf.placeholder(tf.float32, shape=[batch_size, max_sentence_length, nclasses])
...
outputs, states = tf.nn.dynamic_rnn(cell, embedded_chars,
                                    ...
                                    time_major=False,
                                    dtype=tf.float32)
print("shape of outputs: %s" % str(tf.shape(outputs)))
...
outputs2 = tf.matmul(outputs, W_o) + b_o

y_answers = tf.split(1, max_sentence_length, input_y)
y_answers2 = [tf.reshape(i, shape=[batch_size, -1]) for i in y_answers]
y_answers2 = [tf.slice(answer, [0, 0], [sentence_length[i], nclasses]) for i, answer in enumerate(y_answers2)]
